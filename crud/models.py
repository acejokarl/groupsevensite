...
class Genders(models.Model):
    ...
    updated_at = models.DateTimeField(auto_now_add=True) # updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
...
class Users(models.Model):
    ...
    full_name = models.CharField(
        max_length=55, blank=False
    )  # full_name VARCHAR(55) NOT NULL

    gender = models.ForeignKey(
        Genders, on_delete=models.CASCADE
    )  # gender_id BIGINT NOT NULL // FOREIGN KEY(gender_id) REFERENCES tbl_genders(gender_id) ON DELETE CASCADE
    ...
    username = models.CharField(
        max_length=55, blank=False, unique=True
